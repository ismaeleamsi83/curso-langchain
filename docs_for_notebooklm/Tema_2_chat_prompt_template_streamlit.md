# Documentación: Tema 2/chat_prompt_template_streamlit.py

Este documento describe el propósito y funcionamiento del archivo `chat_prompt_template_streamlit.py`.

## Descripción General
Este archivo integra `ChatPromptTemplate` en una aplicación de **Streamlit**, permitiendo al usuario seleccionar la "personalidad" del asistente desde la barra lateral. Esto demuestra la flexibilidad de las plantillas de chat dinámicas.

## Funcionalidades Clave
- **Personalidad Dinámica**: Define un diccionario de mensajes de sistema que cambia según la elección del usuario en el `st.selectbox`.
- **Contexto de Historial**: Convierte los últimos 10 mensajes del historial en un bloque de texto para pasarlo al prompt, manteniendo así la coherencia de la conversación.
- **Streaming con Templates**: Implementa streaming de respuestas utilizando la cadena compuesta por un `ChatPromptTemplate` y un modelo de Gemini.
